<div class="bg-zinc-900 p-8 rounded-3xl border border-zinc-700">
    ...
    <form method="POST" action="<?= htmlspecialchars(base_url('/register'), ENT_QUOTES, 'UTF-8') ?>">
        ...
        <input type="email" name="email" placeholder="Email" class="w-full bg-zinc-800 border border-zinc-700 rounded-2xl px-5 py-4 mb-4" required>
        <input type="password" name="password" placeholder="Password (8+ chars)" class="w-full bg-zinc-800 border border-zinc-700 rounded-2xl px-5 py-4 mb-4" required>
        <?php if (!isset($invitesEnabled) || $invitesEnabled): ?>
            <input type="text" name="invite_code" placeholder="Invite code (1234-5678)" class="w-full bg-zinc-800 border border-zinc-700 rounded-2xl px-5 py-4 mb-6" required>
        <?php endif; ?>
